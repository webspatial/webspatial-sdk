...
export function AnimatedSpatialDiv(props: AnimatedSpatialDivProps) {
  ...
  const positionOriginal = spatialStyleOriginal?.position || {
    x: 0,
    y: 0,
    z: 0,
  }
  ...
  const rotationOriginal = spatialStyleOriginal?.rotation || {
    x: 0,
    y: 0,
    z: 0,
  }
  ...
  const {
    spaceTranslateX = spatialX,
    spaceTranslateY = spatialY,
    spaceTranslateZ = spatialZ,
    spaceRotationX = rotationX,
    spaceRotationY = rotationY,
    spaceRotationZ = rotationZ,
    ...spatialDivProps
  } = otherDivProps
  ...
  const animeTranslateTarget = useRef<{ x: number; y: number; z: number }>({
    ...spatialPosition,
  })
  const animeRotationTarget = useRef<{ x: number; y: number; z: number }>({
    ...spatialRotation,
  })
  ...
  const cycleAnime = (props: AnimatedSpatialDivProps) => {
    ...
    if (
      'spaceTranslateX' in props ||
      'spaceTranslateY' in props ||
      'spaceTranslateZ' in props
    ) {
      animejs({
    ...
    if (
      'spaceRotationX' in props ||
      'spaceRotationY' in props ||
      'spaceRotationZ' in props
    ) {
      animejs({
