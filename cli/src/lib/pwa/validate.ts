...
export function checkManifestJson(manifestJson: Record<string, any>){
    ...
    if (!manifestJson.name && !manifestJson['short_name']) {
      errors.push({code: 3006,
        message: 'In the Spatial Web App Manifest, it is necessary to provide the name property or short_name property (preferably both)',
        // eslint-disable-next-line @typescript-eslint/camelcase
        message_staring_params: {}});
    }
    if (!manifestJson.icons?.length) {
      errors.push({code: 3007,
        message: 'In the Spatial Web App Manifest, the icons property must be provided and it should include at least one icon object',
        // eslint-disable-next-line @typescript-eslint/camelcase
        message_staring_params: {}});
    }
    if (!manifestJson['start_url']) {
      errors.push({code: 3008,
        message: 'In the Spatial Web App Manifest, the start_url property must be provided',
        // eslint-disable-next-line @typescript-eslint/camelcase
        message_staring_params: {}});
    }
    if (!PWAGenerator.DisplayModes.includes(manifestJson.display)) {
      errors.push({code: 3009,
        message: 'In the Spatial Web App Manifest, the display property must be provided, and its value'+
    ...
    if(isNet){
        ...
        if(manifest.start_url.indexOf("https://") == 0){
            ...
            if(urlStart.host !== urlManifest.host){
                ...
                // eslint-disable-next-line @typescript-eslint/camelcase
                message: 'In the Web App Manifest, the start_url must be the same origin with manifest', message_staring_params: {}});
            }
        }
        // start_url必须为https协议
            ...
            throw new CustomError({code: 4000,
                // eslint-disable-next-line @typescript-eslint/camelcase
                message: 'In the Web App Manifest, the start_url must use https', message_staring_params: {}});
        }
    }
}

export async function checkIcons(manifest: Record<string, any>, manifestUrl: string){
    ...
    if(maxSizeImage && !ImageHelper.isFullyOpaque(maxSizeImage)){
        throw new CustomError({code: 4000,
            // eslint-disable-next-line @typescript-eslint/camelcase
            message: 'In the Spatial Web App on VisionPro, must be a fully opaque bitmap.', message_staring_params: {}});
    }
    manifest.icons = [{
        src: maxSizeImageUrl,
