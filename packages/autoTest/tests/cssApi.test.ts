...
describe('CSS API Includes Spatial Support', function (this: Mocha.Suite) {
  ...
  describe('Support JSX In-line style', () => {
    ...
    describe('support Display Property', () => {
      const waitForSpatialized = async () => {
        if (!runner) throw new Error('Puppeteer runner not initialized')
        await runner.waitForFunction(
          `(() => {
      ...
      const setDisplayBySelect = async (value: string) => {
        ...
        await runner.evaluate(v => {
          ...
          const select = optionBlock?.parentElement as HTMLSelectElement | null
      ...
      it('should set display to block when set display to block using inline style', async () => {
        const spatialId = await applyAndAssertDisplay('block', 'block', 'block')
    ...
    describe('support visibility property', () => {
      const waitForSpatialized = async () => {
        if (!runner) throw new Error('Puppeteer runner not initialized')
        await runner.waitForFunction(
          `(() => {
    ...
    describe('support content-visibility property', () => {
      const waitForSpatialized = async () => {
        if (!runner) throw new Error('Puppeteer runner not initialized')
        await runner.waitForFunction(
          `(() => {
      ...
      const applyAndAssertContentVisibility = async (
        ...
        expect((styleInfo as any).inline).to.equal(expectedInline)
    ...
    describe('Support in-line style Material', () => {
      const waitForSpatialized = async () => {
        if (!runner) throw new Error('Puppeteer runner not initialized')
        await runner.waitForFunction(
          `(() => {
      ...
      it('should set material to regular when set material to regular using inline style', async () => {
        await applyAndAssertMaterial('regular')
      })

      it('should set material to translucent when set material to translucent using inline style', async () => {
        await applyAndAssertMaterial('translucent')
      })
    })

    describe('Support in-line style transform', () => {
      it('should set transform to valid value (40px, 30deg) using inline style', async () => {
        if (!runner) throw new Error('Puppeteer runner not initialized')
        await runner.navigate('http://localhost:5173/cssApiTest', {
          waitUntil: 'networkidle0',
          timeout: 30000,
        ...
        await runner.evaluate(() => {
          ...
          const translateX = ranges.find(r => r.min === '-50' && r.max === '50')
          const rotateZ = ranges.find(r => r.min === '-100' && r.max === '100')
      ...
      it('should set transform to different value (50px, 60deg) and transformOrigin to left top using inline style', async () => {
        if (!runner) throw new Error('Puppeteer runner not initialized')
        await runner.navigate('http://localhost:5173/cssApiTest', {
          waitUntil: 'networkidle0',
          timeout: 30000,
      ...
      it('should remove transformOrigin when set transformOrigin to empty string using inline style', async () => {
        if (!runner) throw new Error('Puppeteer runner not initialized')
        await runner.navigate('http://localhost:5173/cssApiTest', {
          waitUntil: 'networkidle0',
          timeout: 30000,
      ...
      it('should set transform to 0(-10px, 45deg), transformOrigin to right bottom using inline style', async () => {
        if (!runner) throw new Error('Puppeteer runner not initialized')
        await runner.navigate('http://localhost:5173/cssApiTest', {
          waitUntil: 'networkidle0',
          timeout: 30000,
