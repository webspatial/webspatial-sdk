 ...
 */
export async function injectWindowProxyBridge(runner: PuppeteerRunner): Promise<void> {
  await runner.evaluateOnNewDocument(() => {
    ...
    (window as any).open = function(...args: any[]): WindowProxy | null {
      ...
      if (windowProxy) {
        ...
        // 为测试目的添加ID
        (windowProxy as any).__TEST_ID__ = `window_proxy_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      }
 ...
 * 创建一个新的windowProxy
 */
export async function createWindowProxy(runner: PuppeteerRunner, url?: string): Promise<string> {
 ...
 */
export async function getWindowProxyIds(runner: PuppeteerRunner): Promise<string[]> {
 ...
 */
export async function evaluateInWindowProxy<T>(runner: PuppeteerRunner, windowProxyId: string, fn: () => T): Promise<T> {
  return await runner.evaluate((id: string, fnStr: string) => {
    const proxies = (window as any).__TEST_WINDOW_PROXIES__ || [];
