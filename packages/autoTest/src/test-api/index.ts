...
import { PuppeteerRunner } from '../runtime/puppeteerRunner';
import { injectPlatformAbilityShim, getRecordedJSBCalls, clearRecordedJSBCalls } from '../adapters/platformAbilityShim';
import { injectWindowProxyBridge, createWindowProxy, getWindowProxyIds, evaluateInWindowProxy } from '../windowProxy/windowProxyBridge';
import { SpatialScene } from '../models/SpatialScene';
import { Spatialized2DElement } from '../models/Spatialized2DElement';
import { SpatializedElement } from '../models/SpatializedElement';
import { Transform, Vec2, Vec3 } from '../models/types';
...
export class TestRunner {
  ...
  private spatialElements: Map<string, SpatializedElement> = new Map();
  ...
  async init(options?: TestRunnerOptions): Promise<void> {
    ...
    // 注册测试回调函数
    await this.runner.expose('test_callNative', async (cmd: string, payload: any) => {
  ...
  async createSpatialized2DElement(): Promise<Spatialized2DElement> {
    ...
    const element = new Spatialized2DElement(this.runner, elementId, windowProxy);
