...
import { Spatialized2DElementProperties, Vec2 } from './types';
...
export class Spatialized2DElement extends SpatializedElement {
   ...
   * 更新2D元素属性
   */
  async updateProperties(properties: Partial<Spatialized2DElementProperties>): Promise<void> {
    ...
    if (properties.scrollEnabled !== undefined) {
      await this.runner.evaluate(
        (elementId: string, scrollEnabled: boolean) => {
          console.log(`Setting scroll enabled for ${elementId}: ${scrollEnabled}`);
