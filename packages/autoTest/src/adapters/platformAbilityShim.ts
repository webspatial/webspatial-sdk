...
import { CommandResult, WebSpatialProtocolResult } from './types';

/**
 * 注入PlatformAbility模拟实现到Puppeteer环境中
 * 这个适配器模拟了CoreSDK中的PlatformAbility接口
 */
export async function injectPlatformAbilityShim(runner: PuppeteerRunner): Promise<void> {
 ...
 * 获取记录的JSB调用
 */
export async function getRecordedJSBCalls(runner: PuppeteerRunner): Promise<string[]> {
 ...
 * 清除记录的JSB调用
 */
export async function clearRecordedJSBCalls(runner: PuppeteerRunner): Promise<void> {
