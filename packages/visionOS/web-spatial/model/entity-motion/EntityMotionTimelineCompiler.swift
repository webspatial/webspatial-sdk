...
extension EntityMotionTimingFunction {
    /// Evaluates the standard cubic Bézier easing at normalized time.
    func evaluate(_ time: Double) -> Double {
        ...
        for _ in 0 ..< 24 {
            ...
            if cubic(parameter, controlPoints.0, controlPoints.2) < clamped {
                lower = parameter
            } else {
                upper = parameter
            }
        }
        return cubic((lower + upper) / 2, controlPoints.1, controlPoints.3)
    }

    /// Evaluates one cubic Bézier coordinate with fixed endpoints zero and one.
    private func cubic(_ parameter: Double, _ first: Double, _ second: Double) -> Double {
