...
struct SpatialWebViewUI: View {
    ...
    @State var viewHeight: CGFloat = .infinity
    var body: some View {
        if let wv = ent.getComponent(SpatialWindowComponent.self) {
            ...
            ZStack(alignment: .top) {
                ...
                ZStack {
                    SpatialWebViewStack(height: viewHeight, isRoot: wv.isRootWebview()) {
                ...
                }.offset(y: hasNav ? NavView.navHeight : 0)
            }
            .opacity(wv.opacity)
            .hidden(!ent.visible)
        }
...
struct SpatialWebViewStack<Content: View>: View {
    let height: CGFloat
    let isRoot: Bool
    @ViewBuilder let childView: () -> Content
    var body: some View {
        if isRoot {
            ZStack {
                childView()
