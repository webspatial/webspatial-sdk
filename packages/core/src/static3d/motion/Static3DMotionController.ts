...
import { getStatic3DMotionSuppressedFields } from './getStatic3DMotionSuppressedFields'
import { warnNativeOnlyMotion } from '../../spatialized/motion/nativeOnlyMotion'

export interface Static3DMotionControllerOptions {
  /** Initial element; may be set later via {@link attachElement}. */
  element?: SpatializedStatic3DElement | null
  /**
   * When set, overrides `supports('useAnimation', ['static3d'])` for backend selection.
...
interface NativeSession {
  ...
  result?: AnimateSpatializedStatic3DResult
  queuedPause?: boolean
  unmounted?: boolean
}
...
function nextMotionObjectId(): string {
  return `__s3motion_${++_motionObjectCounter}_${Date.now()}`
}

let _sessionCounter = 0
function nextAnimationId(): string {
  return `s3motion_${++_sessionCounter}_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
}
 ...
 */
export class Static3DMotionController implements SpatialDivPlaybackApi {
  readonly id: string

  private config: SpatialDivMotionConfig
  private element: SpatializedStatic3DElement | null
  ...
  attachElement(element: SpatializedStatic3DElement | null): void {
  ...
  getSuppressedFields(): Set<string> | null {
    ...
    return getStatic3DMotionSuppressedFields({ ...this.config, tracks: subset })
  }
  ...
  play(): void {
    if (!this.nativeCapable) {
      if (!this.warnedNativeOnly) {
        ...
        warnNativeOnlyMotion('Static3DMotionController', 'static3d')
      }
      return
    }
    this.stopWebRaf()
  ...
  cancel(keys?: SpatialDivMotionPropertyKeys): void {
    ...
    if (normalized && normalized.length > 0) {
      console.warn(
        '[Static3DMotionController] cancel(keys) is not supported; canceling entire session.',
      )
    }
    if (!this.nativeCapable) return
    const ns = this.nativeSession?.state
  ...
  private get nativeCapable(): boolean {
    ...
    return supports('useAnimation', ['static3d'])
  }
  ...
  private buildPlayCommand(
    ...
    elementId: string,
  ): (AnimateSpatializedStatic3DCommand & { type: 'play' }) | null {
  ...
  private reportNativeError(error: {
    ...
    if (cfg.onError) {
      ...
      console.error('[Static3DMotionController] Native error:', error)
    }
  }
  ...
  private async doNativePlay(
    ...
    try {
      const result = await element.animateMotion(cmd)
  ...
  private async nativePlay(): Promise<void> {
    if (!this.nativeCapable) {
      if (!this.warnedNative) {
        ...
        console.warn(
          '[Static3DMotionController] Native motion requires supports(useAnimation, [static3d]).',
        )
      }
      return
    }
    ...
    if (!element) {
      ...
      if (!this.warnedQueued) {
        ...
        console.warn(
          '[Static3DMotionController] Native play is queued until attachElement / motion bind.',
        )
      }
      this.bump()
      return
  ...
  private async nativePause(
    ...
    if (properties && properties.length > 0) {
      console.warn(
        '[Static3DMotionController] Selective native pause is not yet supported; pausing entire session.',
      )
    }
    ...
    if (!element) return
    try {
      const values = await element.animateMotion({
        animationId: session.animationId,
        type: 'pause',
    ...
    if (opts.cancelSession && session && element) {
      if (session.state !== 'idle' && session.state !== 'finished') {
        element
          ...
          .catch(() => {})
        SpatialWebEvent.removeEventReceiver(`${session.animationId}_completed`)
        SpatialWebEvent.removeEventReceiver(`${session.animationId}_canceled`)
        SpatialWebEvent.removeEventReceiver(`${session.animationId}_failed`)
