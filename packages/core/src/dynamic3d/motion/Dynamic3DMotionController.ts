...
export interface Dynamic3DMotionControllerOptions {
  /** Initial element; may be set later via {@link attachElement}. */
  element?: SpatializedDynamic3DElement | null
  /**
   * When set, overrides `supports('useAnimation', ['dynamic3d'])` for backend selection.
...
interface NativeSession {
  ...
  result?: AnimateSpatializedDynamic3DResult
  queuedPause?: boolean
  unmounted?: boolean
}
...
function nextMotionObjectId(): string {
  return `__d3motion_${++_motionObjectCounter}_${Date.now()}`
}

let _sessionCounter = 0
function nextAnimationId(): string {
  return `d3motion_${++_sessionCounter}_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
}
 ...
 */
export class Dynamic3DMotionController implements SpatialDivPlaybackApi {
  readonly id: string

  private config: SpatialDivMotionConfig
  private element: SpatializedDynamic3DElement | null
  ...
  attachElement(element: SpatializedDynamic3DElement | null): void {
  ...
  play(): void {
    if (!this.nativeCapable) {
      if (!this.warnedNativeOnly) {
        ...
        warnNativeOnlyMotion('Dynamic3DMotionController', 'dynamic3d')
      }
      return
    }
    this.stopWebRaf()
  ...
  cancel(keys?: SpatialDivMotionPropertyKeys): void {
    ...
    if (normalized && normalized.length > 0) {
      console.warn(
        '[Dynamic3DMotionController] cancel(keys) is not supported; canceling entire session.',
      )
    }
    if (!this.nativeCapable) return
    const ns = this.nativeSession?.state
  ...
  private get nativeCapable(): boolean {
    ...
    return supports('useAnimation', ['dynamic3d'])
  }
  ...
  private buildPlayCommand(
    ...
    elementId: string,
  ): (AnimateSpatializedDynamic3DCommand & { type: 'play' }) | null {
  ...
  private reportNativeError(error: {
    ...
    if (cfg.onError) {
      ...
      console.error('[Dynamic3DMotionController] Native error:', error)
    }
  }

  private async doNativePlay(
    session: NativeSession,
    element: SpatializedDynamic3DElement,
  ): Promise<void> {
  ...
  private async nativePlay(): Promise<void> {
    if (!this.nativeCapable) {
      if (!this.warnedNative) {
        ...
        console.warn(
          '[Dynamic3DMotionController] Native motion requires supports(useAnimation, [dynamic3d]).',
        )
      }
      return
    }
    ...
    if (!element) {
      ...
      if (!this.warnedQueued) {
        ...
        console.warn(
          '[Dynamic3DMotionController] Native play is queued until attachElement / motion bind.',
        )
      }
      this.bump()
      return
  ...
  private async nativePause(
    ...
    if (properties && properties.length > 0) {
      console.warn(
        '[Dynamic3DMotionController] Selective native pause is not yet supported; pausing entire session.',
      )
    }
