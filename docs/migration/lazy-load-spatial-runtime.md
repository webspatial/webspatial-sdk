...
For **request-time branching** in RSC or middleware (hero for WebSpatial vs plain web), use the **`User-Agent`** header and the **official WebSpatial site documentation** for classification — do **not** depend on SDK-exported runtime-detection helpers as a supported public API. The package may include internal-only modules for WebSpatial demos; they are not part of the documented integrator surface — see [`packages/react/README.md` → "RSC, server requests, and runtime detection"](../../packages/react/README.md#rsc-server-requests-and-runtime-detection).
