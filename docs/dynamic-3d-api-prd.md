...
- **Naming:** `name?` must be a USD-safe identifier. Do not include hyphens (`-`) or other special characters; use camelCase or underscores (`_`) instead. Literal hyphenated names are rejected by the TypeScript API. RCP normalizes unsupported characters to underscores and runtime features such as serialization, USD export, and playback expect consistent names across platforms.
