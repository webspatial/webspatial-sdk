...
| `/server-only-util` | `app/server-only-util/page.tsx` | **Pure RSC.** Calls `detectSpatialRuntime(await headers())` from `@webspatial/react-sdk/server` — the SDK&apos;s server-safe subpath — to branch the initial HTML on the request&apos;s WebSpatial runtime before any client JS executes. Demonstrates the boundary split: hooks/facades live behind `@webspatial/react-sdk` (`'use client'`), pure server-callable helpers live behind `@webspatial/react-sdk/server`. |
