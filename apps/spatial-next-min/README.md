...
| `/server-only-util` | `app/server-only-util/page.tsx` | **Pure RSC.** Imports `getAbsoluteUrl` and `WebSpatialRuntime.supports` from `@webspatial/react-sdk` without crossing the `'use client'` boundary. Proves the spec&apos;s decision to delete the `react-server` exports condition was sound. |
