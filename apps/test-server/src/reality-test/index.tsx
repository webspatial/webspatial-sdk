...
function App() {
  ...
  const [boxSize] = useState({width:0.05, height:0.05, depth:0.05, cornerRadius:0.01})
  ...
  const onTapLeft = async (e: SpatialTapEvent) => {
    console.log('tap left')
    findAndRemoveNearestEntity('left')
  }

  const onTapRight = async (e: SpatialTapEvent) => {
  ...
  const findAndRemoveNearestEntity = (side: 'left' | 'right') => {
    ...
    const sideEntities = entityListRef.current.filter(entity => entity.side === side)
    ...
    if(isPlayGame){
      ...
      function updateGame() {
        ...
        if(particleEntityListRef.current.length){
          let list = particleEntityListRef.current.map(particle => {
  ...
  return (
    <div style={{
      ...
      { !isPlayGame && <>
        <div style={{color:"white", fontSize:"70px", fontWeight:"bold"}}>Let's Saber!</div>
        <div style={{padding: "2em"}}>
          <button style={{marginTop:"40px", width:"200px", height:"80px", fontSize:"30px"}} onClick={onClick}>
            Start!
          </button>
        </div>
      </> }
            ...
            <BoxEntity
             ...
             ))}
             {
              particleEntityList.map((particle, index) => {
                return particle.getPartDatas().map(partData => (
                  <BoxEntity
                    key={partData.id}
                    width={boxSize.width}
                    height={boxSize.height}
