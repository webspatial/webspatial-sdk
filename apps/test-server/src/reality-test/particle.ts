            ...
            for(var j = 0; j < this.layer; j++){
                for(var k = 0; k < this.layer; k++){
                    ...
                    let entityInfo = {
                        ...
                        position: {x:x + this.originPos.x, y:y + this.originPos.y, z:z + this.originPos.z},
                    ...
                    this.partLifeFrames.push(Math.ceil((Math.random() * 0.8 + 0.2) * this.duration))
                }
            }
        }
    }
            ...
            for(var j = 0; j < this.layer; j++){
                for(var k = 0; k < this.layer; k++){
                    ...
                    if(this.frame < this.partLifeFrames[partIndex]){
                        fadePercent = Math.max(0, Math.min(this.frame / this.partLifeFrames[partIndex], 1))
                        ...
                        fadeNormal.x += dirPos.x * this.partSpeeds[partIndex] + this.breakDir.x * this.breakForce
                        fadeNormal.y += dirPos.y * this.partSpeeds[partIndex] + this.breakDir.y * this.breakForce
                        fadeNormal.z += dirPos.z * this.partSpeeds[partIndex] + this.breakDir.z * this.breakForce
                        this.partDatas[partIndex].position.x = x * 1.2 + 0.5 * this.partSize + this.originPos.x + fadeNormal.x * fadePercent
                        this.partDatas[partIndex].position.y = y * 1.2 - 0.5 * this.partSize + this.originPos.y + fadeNormal.y * fadePercent
                        this.partDatas[partIndex].position.z = z * 0.5 + 0.5 * this.partSize + this.originPos.z + fadeNormal.z * fadePercent
