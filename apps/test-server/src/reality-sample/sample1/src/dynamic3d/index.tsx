...
import { BoxEntity, ModelAsset, ModelEntity, Reality, SceneGraph, UnlitMaterial } from "@webspatial/react-sdk"
...
function DynamicTest(){
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < barCount; i++) {
            ...
            let entityInfo = {
                id: `entity_${i}`,
                position: {
                    x: (i * barWidthRef.current - canvas.width / 2 + barWidthRef.current ) / scale / window.devicePixelRatio,
                    y: 0,
                    z: 0.2,
                },
    ...
    useEffect(() => {
        function updateGame() {
            ...
            barHeightRef.current += (targetHeightRef.current - barHeightRef.current) * 0.03
            const maxHeight = barHeightRef.current
            const barWidth = barWidthRef.current
            
            const updatedEntities = [...entityListRef.current];
            for (let i = 0; i < barCount; i++) {
                const waveHeight = Math.abs(Math.sin(i * Math.PI / barCount + frameRef.current * speed)) * maxHeight;
                const x = i * barWidth;
                ...
                updatedEntities[i] = {
                    ...
                    position: {
                        ...
                        y: (waveHeight - canvas.height / 2) / scale / window.devicePixelRatio
                    }
                };
            }
            ...
            let preCarX = Math.sin((carFrameRef.current-1) * speed) * .4
            let carX = Math.sin(carFrameRef.current * speed) * .4
            let carY = Math.abs(Math.sin((carX + .4) / .8 * Math.PI + frameRef.current * speed)) * maxHeight;
            const angle = carX - preCarX > 0 ? 90 : -90
            setBoxRot({ x: 0, y: -angle, z: 0 })
            setBoxPos({ x: carX, y: (carY - canvas.height / 2) / scale / window.devicePixelRatio + 0.05, z: 0.2 })
            ...
            <div style={{
                ...
                >
                    {materialList.map(material => (
                        <UnlitMaterial key={material.id} id={material.id} color={material.color} />
                    ))}
