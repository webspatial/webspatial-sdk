...
export default function EntityAnimationCapabilityCheckPage() {
  const supported = WebSpatialRuntime.supports('useEntityAnimation')

  return (
    <EntityAnimationPageShell
      title="Capability Detection"
      description="Check whether the current runtime reports support for the useEntityAnimation hook."
    >
      <section className="rounded-2xl border border-gray-800 bg-[#111] p-6">
        <p className="text-sm text-gray-400">
          <code className="text-cyan-300">
            WebSpatialRuntime.supports('useEntityAnimation')
          </code>
        </p>
        <div
        ...
        >
          supports('useEntityAnimation') = {String(supported)}
        </div>
      </section>
    </EntityAnimationPageShell>
  )
