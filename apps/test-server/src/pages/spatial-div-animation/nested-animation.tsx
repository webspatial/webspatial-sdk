...
function InnerScene({ log }: { log: (msg: string) => void }) {
  const [animation, api] = useAnimation({
    ...
    onComplete: (values: any) =>
      log(`[inner] onComplete → ${fmtValues(values)}`),
    onCancel: (values: any) => log(`[inner] onCancel → ${fmtValues(values)}`),
    onError: (err: any) => log(`[inner] onError → ${err.reason}`),
  } as any)
...
function MiddleLayer({ log }: { log: (msg: string) => void }) {
  const [animation, api] = useAnimation({
    ...
    onComplete: (values: any) =>
      log(`[middle] onComplete → ${fmtValues(values)}`),
    onCancel: (values: any) => log(`[middle] onCancel → ${fmtValues(values)}`),
    onError: (err: any) => log(`[middle] onError → ${err.reason}`),
  } as any)
...
function OuterLayer({
  ...
  const [animation, api] = useAnimation({
    ...
    onCancel: (values: any) => log(`[outer] onCancel → ${fmtValues(values)}`),
    onError: (err: any) => log(`[outer] onError → ${err.reason}`),
  } as any)
        ...
        <div className="mt-6 rounded-xl border border-gray-700 bg-black/30 p-4 text-xs text-gray-400">
          ...
          <ul className="list-disc space-y-1 pl-5">
            ...
            <li>
              ...
              onComplete / onCancel fires independently (check log below).
            </li>
            <li>
              <strong>Unmount</strong>: "Unmount Middle + Inner" → Middle's
              onCancel fires, Inner unmounts, Outer continues unaffected.
            </li>
          </ul>
        </div>
