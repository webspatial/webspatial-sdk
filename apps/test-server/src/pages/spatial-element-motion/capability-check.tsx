...
export default function SpatialDivCapabilityCheckPage() {
  const supported = WebSpatialRuntime.supports('useSpatializedMotion', [
    'element',
  ])
  const entitySupported = WebSpatialRuntime.supports('useSpatializedMotion', [
    'entity',
  ])
  ...
  return (
    <SpatialDivAnimationPageShell
      ...
      description="Probe the runtime for Spatialized Element Motion support. The element token is independent of the entity token."
    >
      <section className="rounded-2xl border border-gray-800 bg-[#111] p-6 space-y-4">
        <div>
          <p className="text-sm text-gray-400">
            <code className="text-cyan-300">
              WebSpatialRuntime.supports('useSpatializedMotion', ['element'])
            </code>
          </p>
          <div
          ...
          >
            supports('useSpatializedMotion', ['element']) = {String(supported)}
          </div>
        </div>
        ...
        <div>
          <p className="text-sm text-gray-400">
            <code className="text-cyan-300">
              WebSpatialRuntime.supports('useSpatializedMotion', ['entity'])
            </code>{' '}
            (for comparison)
          </p>
          <div
          ...
          >
            supports('useSpatializedMotion', ['entity']) ={' '}
            {String(entitySupported)}
          </div>
        </div>
