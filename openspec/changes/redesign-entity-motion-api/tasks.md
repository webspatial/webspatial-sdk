...
- [ ] 4.4 Implement committed-state ownership rules: `api.set(values)` only accepts sparse patch objects, native merges patches over the current committed transform baseline, dynamic take-over after inactive playback uses `api.set` rather than competing React prop writes, calling `api.set` during an active animation is not stashed or replayed and does not update `entityProps`, calling `api.set` before binding or native object creation is invalid, a fresh play after set takes declared fields from the config start boundary and fills omitted fields from the latest committed transform, and terminal fill writes the terminal transform back to `entityProps` (fill-forwards, no snap-back)
...
- [ ] 4.8 Add failing tests for fresh-play compilation semantics: create only validates and stores the canonical timeline; first `play` / `autoStart` and `play` after `complete` / `finish` / `stop` / `reset` read the latest native transform and recompile; `play` after `pause` and loops within one run reuse the current resource without reading a new baseline; compilation failure fails the control command and remains inactive
- [ ] 4.9 Implement native fresh-play compilation: create registers an animation object that owns the canonical timeline; each accepted fresh play reads the baseline and generates the RealityKit resource before entering delay / running; resume after pause reuses the current controller; loops reuse the current run's resource
...
- [ ] 7.2 Run targeted unit, integration, and capability tests for Entity motion and runtime capabilities, including `api.set(values)` patch-only behavior, active set not stashed, unbound set invalid, set then fresh play using the latest baseline, replay after terminal state recompiling, pause/resume and loop reusing the current resource, fresh-play compilation failure remaining inactive, and terminal fill; and covering top-level `from` / `to` equivalence, `timeline` precedence warning, default 0.3s duration, rejection when any shape is missing a start/end boundary (including removal of "no start frame → start from current pose"), and field-level missing-keyframe baseline fallback within boundary frames
