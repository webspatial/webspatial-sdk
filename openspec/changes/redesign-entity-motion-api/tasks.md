...
- [x] 4.3 Add failing tests for the public playback API, creation-pending playback queue, and whole-transform write protection: native animation-object creation with no pending playback command remains `idle`; a playback command waiting for creation, including the implicit `autoStart` play, exposes `queued`; playback commands before creation flush in call order with `autoStart` first; while commands are queued, `isAnimating`, `isPaused`, and `finished` remain `false`; a successful creation reply confirms `idle` before flushing; a queued `pause` or `stop` preserves `idle` without a state event; a failed creation reply settles `idle`, terminates the current binding lifecycle, clears object state, pending commands, and `entityProps`, leaves the remaining React props in control, and dispatches one classified error; pre-creation `set` and every API after binding termination remain warning + no-op; post-creation `set → play`, `stop → play`, and `play → pause` submit immediately in call order and rely on JSB/Native FIFO; active-playback `set` reaches Native in FIFO order and maps `INVALID_CONTROL_STATE` to warning + no-op; each command failure reports its own error while later commands continue; unbind, replacement, and destruction clear pending creation playback commands; active-playback writes preserve the animation and latest `entityProps`; fresh play enables protection, pause retains it, stop/reset/finish/natural completion remove it, and inactive ordinary React transform updates reach Native; terminal fill preserves the committed pose through complete `entityProps`
- [x] 4.4 Implement the React creation-pending playback queue and direct Core command submission; use the `CreateEntityAnimation` reply to confirm `idle` before flushing or terminate the current binding lifecycle on creation failure; keep pre-creation `set` and every post-termination command outside the queue as warning + no-op; submit every post-creation command immediately and rely on JSB/Native FIFO; then implement JSB command initiation and complete-transform `entityProps` updates so combined React props control inactive playback; Section 5 implements native set merging, the state machine, terminal commits, and transform write protection
- [x] 4.4b Add Core/React red tests for one state-message shape, authoritative `playState` updates, paired `callbackAction` plus complete `values`, state-only pause and resume, shared `onComplete` dispatch for `finish()` and natural completion, immediate command submission with JSB/Native FIFO, and state-message/reply races preserving the latest state
- [x] 4.4c Implement Core/React state-message consumption: `playState` updates public state, optional `callbackAction` dispatches callbacks and confirmed values, and successful control replies confirm each submitted command while JSB/Native FIFO preserves order
- [x] 4.4d Add Core/React target-destruction red tests for animation-id `objectdestroy`, destroyed state, event-receiver removal for that id, later playback as a local no-op, later `set` as a local warning plus no-op, stable JSB-call count, and stable `onError` count
...
- [x] 8.2 Run targeted React/Core tests for the tuple, binding, normalization, capability detection, `callbackAction`, state-message/control-reply races, target `objectdestroy`, post-destroy local no-op guards, `entityProps`, transform write protection, inactive React transform updates, and `api.set` command initiation
...
- [x] 9.2 Add React tests for stable object and id, creation-pending playback queue, direct post-creation command submission, callback updates, `autoStart`, and failure recovery; verify revision filtering in Core
...
- [x] 9.8 Minimally implement React in-place update and creation-pending playback queue; submit post-creation updates directly through Core and delete same-target recreate, pose handoff, and replacement generation
...
- [x] 10.1 Add Core red tests for `api.set` during object destruction and after `objectdestroy`, covering local warning + no-op behavior for invalid and valid updates with stable JSB-call and `onError` counts.
