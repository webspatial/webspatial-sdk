...
- [ ] 4.3 Add failing tests for the public playback API and whole-transform ownership: `api.set` accepts sparse patch objects; calls before binding or native animation-object creation preserve the current state; active-playback writes preserve the animation and the latest `entityProps`; after confirmation the complete `entityProps` mirror remains authoritative while the binding is attached; unbinding returns control to React props; terminal fill preserves the committed end pose
- [ ] 4.4 Implement the React/Core playback API, JSB command initiation, complete-transform outlet ownership, and authority return on unbind; Section 5 implements native set merging, the state machine, and terminal commits
