...
- 同一目标的归一化执行签名发生变化时,绑定对象保持旧对象和旧代次至销毁成功。旧对象销毁成功并释放 transform owner 后,绑定对象通过普通 Entity transform 更新入口提交当前 `entityProps` 中最近一次确认的完整姿态,并等待该更新成功,随后推进绑定代次并使用最新配置创建新对象。该姿态恢复保持现有 `entityProps` 和生命周期 callback 次数。替换对象的首次 fresh play 读取该确认姿态作为基准姿态。旧对象销毁失败时,绑定对象保持旧对象和旧代次,清理本次替换产生的待执行命令,并触发一次 `onError`。
...
| 同目标配置替换期间姿态与镜像出现差异 | 同目标替换依次完成旧对象销毁、最近确认姿态恢复和新对象创建,4.3a 覆盖完整交接屏障 |
