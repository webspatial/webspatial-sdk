...
新的 Entity motion 目标态 capability 契约 MUST 使用顶层 `supports('useEntityAnimation')`。`supports('useEntityAnimation', ['entity'])` MUST 从文档化的 capability 契约中移除:任何文档、目标态指引或保留 sub-token MUST NOT 再描述 `useEntityAnimation` 的 `entity` sub-token。
