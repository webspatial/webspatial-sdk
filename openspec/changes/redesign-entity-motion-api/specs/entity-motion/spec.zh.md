...
公开 Entity motion 状态 MUST 使用 `queued`、`idle`、`running`、`paused` 和 `finished`。`queued` MUST 表示原生动画对象创建前的 React 绑定阶段。`queued` 期间 `isAnimating`、`isPaused` 和 `finished` MUST 保持 `false`,排队命令 MUST 保持这些布尔值。原生层创建回执、控制回执和状态事件 MUST 作为公开播放状态与布尔值的唯一数据源。原生层状态 MUST 使用 `idle`、`running`、`paused` 和 `finished`。原生层创建成功回执 MUST 在执行待处理命令前确认初始 `idle` 状态。原生层创建失败回执 MUST 使公开状态收敛为 `idle`、清理待处理命令并进入分类错误链路。公开 `finished` 标记 MUST 等于 `playState === 'finished'` 的结果。
...
- **AND** 失败回执 MUST 确认公开 `idle`、清理待处理命令并分发分类错误
...
- **AND** 替换对象的首次 fresh play MUST 读取当前原生 transform 作为基准姿态,其中包括已恢复的确认姿态
- **AND** 该姿态恢复 MUST 保持现有 `entityProps` 和生命周期 callback 次数

#### Scenario: 同一目标替换时姿态恢复或创建失败
- **GIVEN** 同一目标替换中的旧动画对象已经销毁成功
- **WHEN** 确认姿态恢复或替换对象创建失败
- **THEN** 原生层失败回执 MUST 结束本次替换并使公开播放状态收敛为 `idle`
- **AND** SDK MUST 清理本次替换产生的待执行命令
- **AND** SDK MUST 保持当前 `entityProps`
- **AND** `onError` MUST 触发一次
- **AND** 后续执行签名变化 MUST 能够再次创建对象
...
动画处于活跃播放状态时,动画系统 MUST 持有完整的物体变换控制权。底层平台(visionOS / picoOS)绑定整个 `.transform`;配置字段执行动画,其余字段 MUST 保持基准姿态。此期间活动动画 MUST 保持控制权,最新的 `entityProps` 已确认值 MUST 保持稳定,SDK MUST 立即丢弃 React 属性写入和 `api.set` 写入。首个已确认状态产生后,播放空闲时的动态写入 MUST 使用 `api.set`;普通 React 变换属性在解绑前保持为基础输入。Native `SpatialScene` MUST 在普通 Entity transform 更新入口执行完整 transform 的 animating mask 仲裁。首个已确认状态产生前,普通更新 MUST 更新 fresh play 的 baseline;首个已确认状态产生后且绑定存续期间,普通更新 MUST 返回成功并保持当前原生 transform;解绑或销毁动画对象后,普通更新 MUST 恢复更新原生 transform。
