...
- **Own the whole transform through the binding lifecycle.** Once the animation is active, the entire `.transform` is owned by the animation. For example, animating only `position.y` freezes `position.x` / `position.z` — and `rotation` / `scale` too — at baseline during playback. After native produces the first confirmed state, `entityProps` persists the complete committed transform while the binding remains attached; inactive dynamic writes use `api.set`, and unbinding returns control to React props. Entity motion reuses the Element animation's native animating-mask arbitration: the target Entity stores the whole-transform animation owner, `EntityMotionAnimationObject` acquires that owner when it commits the first confirmed state, and `SpatialScene` checks the owner at the ordinary Entity transform update entry. While the owner exists, an ordinary update returns success and preserves the current native transform; unbinding or destroying the animation object releases the owner.
...
- A normalized execution-signature change on the same target keeps the old object and generation current until destruction succeeds. After the old object is destroyed and releases the transform owner, the binding restores the latest confirmed complete pose through the ordinary Entity transform update entry when `entityProps` contains one. When `entityProps` is empty, the current native transform remains authoritative and the binding proceeds directly to new-object creation. The binding then advances the generation and creates the new object from the latest config. The replacement object's first fresh play reads the current native transform, including any restored confirmed pose, as its baseline. Pose restoration preserves the existing `entityProps` and lifecycle callback counts. A failed pose-restoration or new-object-creation reply ends the attempted replacement, settles the public state to `idle`, clears its pending commands, preserves `entityProps`, and fires `onError` once; a later execution-signature change may retry creation. When old-object destruction fails, the binding retains the old object and generation, clears commands pending for the attempted replacement, and fires `onError` once.
...
| Pose and mirror diverge during same-target config replacement | Same-target replacement branches on the presence of a confirmed pose, keeps the current native transform authoritative for an empty mirror, and settles restoration or creation failures through native replies; 4.3a covers the complete handoff barrier |
