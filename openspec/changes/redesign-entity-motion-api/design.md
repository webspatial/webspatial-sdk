...
- A normalized execution-signature change on the same target keeps the old object and generation current until destruction succeeds. After the old object is destroyed and releases the transform owner, the binding submits the latest confirmed complete pose from the current `entityProps` through the ordinary Entity transform update entry and waits for that update to succeed, then advances the binding generation and creates the new object from the latest config. This pose restoration preserves the existing `entityProps` and lifecycle callback counts. The replacement object's first fresh play reads that confirmed pose as its baseline. When old-object destruction fails, the binding retains the old object and generation, clears commands pending for the attempted replacement, and fires `onError` once.
...
| Pose and mirror diverge during same-target config replacement | Same-target replacement completes old-object destruction, latest-confirmed-pose restoration, and new-object creation in order; 4.3a covers the complete handoff barrier |
