...
The native backend of `useSpatialDivMotion` MUST be enabled only when `supports('useAnimation', ['element'])` is `true`. The Web backend MUST NOT depend on that capability.

#### Scenario: Capability false still allows Web motion

- **GIVEN** `supports('useAnimation', ['element'])` is `false`
- **WHEN** `useSpatialDivMotion` is used with `api.play()`
