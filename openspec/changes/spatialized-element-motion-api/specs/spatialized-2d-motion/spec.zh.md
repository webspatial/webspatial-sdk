...
- **WHEN** 开发者调用 `useSpatializedMotion(config)` 或 `useSpatializedMotion.simple(config)` 并将 `animation` 绑定到 `enable-xr` 节点
...
- **WHEN** 应用调用 `useSpatializedMotion.simple({ from, to, duration, ... })` 并绑定到 `enable-xr` 节点
