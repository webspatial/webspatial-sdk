...
实现 MUST 使用 `SpatializedMotionController`，当 `animation` 绑定到 `<Reality>` 组件时解析为 `dynamic3d` 目标（仅 native；无 Web RAF）。
...
- **WHEN** `<Reality xr-animation={binding} />` 接收来自 `useSpatializedMotion(config)` 的 `animation`，目标解析为 `dynamic3d`
