...
- **WHEN** `stop()`、`reset()` 或 `finish()` 完成且 transform suppression 被释放
...
- **WHEN** `stop()`、`reset()` 或 `finish()` 之后 suppression 被释放
- **THEN** SDK MUST 避免进入 native host transform 与 DOM 或 Portal host transform 在终态后同时继续控制同一视觉 `transform` 的状态
...
- **AND** 新的 active 会话 MUST 按目标类型重新进入 transform suppression
