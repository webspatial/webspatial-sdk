...
- **WHEN** 开发者调用 `useSpatializedMotion(config)` 并将 `animation` 通过 `xr-animation` prop 传给组件
...
公开 hook `useSpatializedMotion(config)` MUST NOT 要求 config 中有 `kind` 字段。返回的 `animation` binding MUST 携带延迟目标。目标解析 MUST 在 binding 作为 `xr-animation` prop 传给组件时发生：
...
- **WHEN** `useSpatializedMotion(config)` 返回的 `animation` 作为 `xr-animation` 传给 `<div enable-xr>`
