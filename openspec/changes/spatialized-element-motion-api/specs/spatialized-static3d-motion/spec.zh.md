...
实现 MUST 使用 `SpatializedMotionController`，`kind: 'static3d'`（仅 native；无 Web RAF）。
...
- **WHEN** `<Model motion={binding} />` 从 `useSpatializedMotion({ kind: 'static3d' })` 接线
...
- **THEN** timeline `motion` 会话 MUST NOT 被隐含启动；二者为独立 API
