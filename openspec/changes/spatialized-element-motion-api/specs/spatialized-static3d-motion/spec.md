...
### Requirement: Static3D timeline animates container root transform and opacity

The SDK MUST support `SpatializedStatic3DElement` timeline motion applying sampled values to container-root `transform` (translate/rotate/scale) and root `opacity` without animating model-internal fields.
...
Static3D timeline motion MUST NOT drive model-internal `entityTransform` / `modelTransform` playback fields.
...
- **THEN** native MUST sample the timeline and update container-root `transform` and `opacity` until completion or session termination
- **AND** native MUST NOT update model-internal `entityTransform` / `modelTransform`
...
- **THEN** play before bind MAY queue; after bind native playback MUST drive container-root `transform` / `opacity` without fighting React layout writes through the element animating mask
