...
<Model src="robot.usdz" style={{ ...style }} xr-animation={animation} /> // → Static3D 容器
<Reality style={{ ...style }} xr-animation={animation}><Entity /></Reality> // → Dynamic3D 容器

// style 行为：
// 2D：作为合并/快照出口；必须合并回绑定宿主，纯 Web runtime 不启动 playback
// 3D：同样必须合并回绑定宿主，以形成状态闭环；播放仍由 xr-animation 绑定交给 native 驱动
```
...
补充约束：
- 所有通过宿主节点绑定 `xr-animation` 的目标都 MUST 将返回的 `style` 合并回同一宿主，以形成动画值 → 宿主样式 → 后续重同步的一致闭环。
- 不合并 `style` 时，播放 MAY 仍然开始，但 `stop()`、`reset()`、`finish()`、自然结束后的视觉状态持久性不受保证。
...
- 对所有宿主绑定目标，终止方法返回的视觉值只有在调用方将返回的 `style` 合并回宿主后，才构成对后续 rerender / resync 稳定可见的终态
...
| `supports('useAnimation')` | 发布后的 motion API 唯一能力 gate；覆盖容器 motion 与当前 `useEntityAnimation` 栈 |
...
- 纯 Web runtime 下 `supports('useAnimation')` 返回 `false`，且目标态路径不启动 playback fallback
...
- capability 检测使用 `supports('useAnimation')`
- `element` / `static3d` / `dynamic3d` 仅作为内部目标解析 kind，不作为 capability sub-token
- 所有宿主绑定目标都要求调用方将返回的 `style` 合并回绑定宿主
