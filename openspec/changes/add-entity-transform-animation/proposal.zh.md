...
Hook 声明动画意图，播放由 Native 以 90 fps 执行。需要时可通过 `api.play()`、`pause()`、`cancel()` 命令式控制；其中 paused 后再次 `play()` 会继续当前会话，并通过 `onError` 接收 bridge/native 异步失败。Native 侧状态变化是异步回写的，因此在同一个 tick 里刚调用完命令后立刻读取 `api.playState` / `api.finished`，仍可能读到上一次已提交的状态，直到对应的 native 事件回来为止。
...
- 明确 `animation` prop 的作用域，区分静态保证与运行时行为：
  ...
  - **运行时行为（entity 未绑定时）**：若带有 `useAnimation` 的组件渲染在 `Reality` / `SceneGraph` 之外（即从未绑定到 `SpatialEntity` 上下文），行为如下：
    ...
    - 对于已经绑定的 native 会话，`playState` / `finished` 不是在命令返回时同步更新，而是在对应的 native 状态事件回来后更新；只有未绑定的 queued 路径会立即更新本地状态。
