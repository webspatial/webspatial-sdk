...
- 明确 `animation` prop 仅被接入 `SpatialEntity` 抽象的 Entity 组件（如 `BoxEntity`、`ModelEntity`）接受，不适用于 `SpatialDiv` 或非 Entity 组件；该限制通过 TypeScript 类型定义在编译期静态保证。在运行时，若 `useAnimation` 用于从未渲染在 `Reality` / `SceneGraph` 下（即从未绑定到 `SpatialEntity` 上下文）的实体，播放将进入 `queued` 状态，直到实体绑定或被卸载。
