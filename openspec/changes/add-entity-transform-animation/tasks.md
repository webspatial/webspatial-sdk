...
- [x] 2.2 Wire completion / stop / failure event handling so JS receives native terminal transform values or error payloads for callbacks and state sync *(blocked by 2.1)*
...
- [x] 3.1 Implement `useAnimation(config)` with `play`, `pause`, `resume`, `stop`, `isAnimating`, `isPaused`, `onStart`, `onError`, `autoStart`, `delay`, and `loop` behavior *(blocked by 2.1)*
...
- [x] 5.2 Update the relevant docs in `docs/` and any representative examples or test-server pages for the new animation API
