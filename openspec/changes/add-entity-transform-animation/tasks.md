...
- [x] 1.5 Add `api.finished` boolean to `AnimationApi` that reflects whether the most recent session completed naturally
- [x] 1.6 Add `api.playState` to `AnimationApi` returning `'idle' | 'queued' | 'running' | 'paused' | 'finished'`; `delaying` internal state maps to `'running'`
...
- [x] 3.1 Implement `useAnimation(config)` with `play`, `pause`, `cancel`, `isAnimating`, `isPaused`, `finished`, `onStart`, `onError`, `autoStart`, `delay`, and `loop` behavior, with `play()` resuming the current session after pause *(needs realign: cancel restores `from`)* *(blocked by 2.1)*
...
- [x] 4.2 Implement native play, pause, resume, and cancel behavior plus completion / cancel / failure event emission with transform or error payloads *(needs realign: cancel restores `from` and changes canceled payload semantics)* *(blocked by 2.2, 4.1)*
- [x] 4.3 Verify delay and loop semantics match the OpenSpec contract for reset looping and reverse looping
- [x] 4.4 Map `playbackRate` from the bridge command to `AnimationView.speed` on the native AVP side
...
- [x] 5.1 Add focused tests for:
  ...
  - [x] 5.1.6 Animation prop replacement (cancel old session → start new, onCancel before onStart, cancel restores `from`) *(needs realign)*
  ...
  - [x] 5.1.9 Bridge failure recovery (session retains pre-failure state, onError invoked or console.error fallback, no completed/canceled after failed play)
  - [x] 5.1.10 cancel-old failure blocks start-new (both restart and animation-prop replacement paths)
  - [x] 5.1.11 `playbackRate` validation (reject 0, negative, NaN, Infinity; default 1) and native speed mapping
  - [x] 5.1.12 `api.finished` state transitions (false initially, true after natural completion, reset to false on next `play()`, stays false after `cancel()`)
  - [x] 5.1.13 `api.playState` value correctness across all states (idle→queued→running→paused→running→finished, and cancel→idle)
- [x] 5.2 Update the relevant docs in `docs/` and any representative examples or test-server pages for the new animation API *(needs realign: cancel restores `from`)*
