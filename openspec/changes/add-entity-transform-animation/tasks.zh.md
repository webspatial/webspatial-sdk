...
- [x] 2.2 打通 completed / stopped / failed 事件处理，使 JS 能拿到 Native 终态 transform 或错误 payload 以触发回调并同步状态 *(blocked by 2.1)*
- [x] 2.3 扩展 capability keys 与数据表，使 `supports('useAnimation')` 能按 runtime 正确解析

## 3. React SDK 集成

- [x] 3.1 实现 `useAnimation(config)`，支持 `play`、`pause`、`resume`、`stop`、`isAnimating`、`isPaused`、`onStart`、`onError`、`autoStart`、`delay`、`loop` *(blocked by 2.1)*
...
- [x] 5.2 更新 `docs/` 中相关文档，以及示例或 test-server 页面以覆盖新的动画 API
