...
- 将动画属性范围明确限制为需求定义的白名单：`back`、`transform.x/y/z`、`opacity`、`depth`、`width`、`height`。
- 约定 `transform` 动画仅覆盖平移分量 `x/y/z`，不在本次提案中支持 `SpatialDiv` 的旋转、缩放或任意 CSS transform 字符串插值。
...
- `spatial-div-animation`：为 `SpatialDiv` 提供声明式与命令式动画能力，覆盖 `back`、`transform.x/y/z`、`opacity`、`depth`、`width`、`height` 的白名单字段，以及生命周期、错误回调和 React 集成规则。
