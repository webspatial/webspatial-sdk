...
- `runtime-capabilities`：新增并文档化 `supports('useSpatialDivAnimation')` 能力 key，用于在运行时判断 `SpatialDiv` 动画是否可用。
...
- **Runtime Capabilities**：需要补充 `supports('useSpatialDivAnimation')` 的解析、文档和测试。
