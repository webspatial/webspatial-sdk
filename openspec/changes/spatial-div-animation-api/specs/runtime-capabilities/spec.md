...
The runtime capability API MUST document and resolve the top-level key `useSpatialDivAnimation`, indicating whether the current runtime supports `SpatialDiv` whitelisted-property animation end-to-end.
...
- **THEN** `supports('useSpatialDivAnimation')` MUST return `true`

#### Scenario: Unsupported runtime

- **WHEN** the runtime lacks any critical capability required for `SpatialDiv` animation
- **THEN** `supports('useSpatialDivAnimation')` MUST return `false`

#### Scenario: Stable supports results

- **WHEN** application code calls `supports('useSpatialDivAnimation')` multiple times within the same runtime process lifetime
...
- **THEN** `supports('useAnimation')` and `supports('useSpatialDivAnimation')` MUST independently return their corresponding results
