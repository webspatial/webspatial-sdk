...
- **WHEN** 应用在同一 runtime 进程生命周期内多次调用 `supports('useAnimation', ['element'])`
- **THEN** 返回结果 MUST 保持稳定，不得在运行时变化

#### Scenario: 与实体动画能力独立

- **WHEN** 某个 runtime 支持实体 transform 动画但不支持 `SpatialDiv` 动画，或反之
- **THEN** `supports('useAnimation', ['entity'])` 与 `supports('useAnimation', ['element'])` MUST 各自独立返回对应结果
- **AND** 一个 sub-token 的结果 MUST NOT 隐式推导另一个 sub-token 的结果
