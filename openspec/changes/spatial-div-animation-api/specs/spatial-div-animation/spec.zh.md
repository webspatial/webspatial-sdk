...
`SpatialDiv` 动画 MUST 仅支持以下白名单字段：`back`、`transform.x`、`transform.y`、`transform.z`、`opacity`、`depth`、`width`、`height`。
...
- **THEN** SDK MUST 仅接受 `x`、`y`、`z` 三个平移分量
...
| `transform.x/y/z` | 有限值 | `NaN`、`Infinity` MUST 被拒绝 |
