...
- **BREAKING** Remove four internal container / monitor identifiers from the default entry's public exports: `SpatializedContainer`, `Spatialized2DElementContainer`, `SpatializedStatic3DElementContainer`, `SpatialMonitor`. These were never intended as public API — the supported usage is via the HOCs `withSpatialized2DElementContainer(Comp)` and `withSpatialMonitor(Comp)`. They continue to live inside `@webspatial/react-sdk/spatial` as facade / HOC implementation details. (The `Entity` base class remains a public facade because it is a documented base for user-defined entities.)
