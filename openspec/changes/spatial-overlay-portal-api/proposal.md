...
判断方式（Phase B MVP）：**菜单 `Content` 里有没有内层 `div enable-xr`**。

Phase C 起推荐显式 **`<SpatialOverlay>`** 替代隐式 `enable-xr` + Radix signal 猜测；`SpatialOverlay` **必须支持递归**（overlay 内再开 overlay）。
...
**Phase C 目标路径** — 显式 `SpatialOverlay` + Radix；在 auto portal routing 前仍使用 `useSpatialPortalContainer()`：
...
### Phase C — `SpatialOverlay`（含递归，已决策 2026-06-10）

- 显式 `<SpatialOverlay>` 替代隐式 `div enable-xr` overlay 猜测；**对用户独立组件**，内部薄包装 `SpatializedContainer(overlayPortalMode)`，用户不写 `enable-xr`
- **支持递归**：`SpatialOverlay` 内可再嵌套 `SpatialOverlay`（如 Radix submenu）；内层 visible surface 挂外层 visible overlay，不被裁剪
- **嵌套 Radix Portal 约定**：内层 `Portal container={useSpatialPortalContainer()}` 必须指向**父 visible overlay webview**（非根 Parent SpatialDiv webview）
- measure tree 渲染完整 children + `OverlayRenderModeContext` gate；visible tree 承担全部 native surface 创建
- Phase C **在 Phase B AVP smoke（§5.9）通过后**启动
- 键盘 / focus / outside-click 仍不在范围
...
### Phase B — Scenario 3（已实现，非递归 MVP）

- 让 **nested `enable-xr` + Radix** 达到上文 Scenario 3 的可观测保证（单层 flat menu items）
- demo 增加 Scenario 3 面板，代码形态与「过渡开发者 API」一致
- 实现：Path A 双渲染 + Radix signal 自动识别 — 见 `design.md` §6/§8

**限制**：hidden measurement host 直接双渲染真实 children；若 children 内含 nested `enable-xr` 或 nested overlay，measure tree 会触发 spatial side effects。**不适合递归**。

### Phase C — `SpatialOverlay` 显式 API + 递归（本 change 后续 slice）

- 把 Phase B 隐含 overlay 逻辑收敛为公开 **`<SpatialOverlay>`** 组件
- 引入 **`OverlayRenderModeContext`**（`measure` | `visible`）分离 measurement tree 与 visible tree
- **必须支持递归**：`SpatialOverlay` 内部可再嵌套 `SpatialDiv` / `SpatialOverlay`（例如 overlay 菜单里再开子菜单）
- Radix `data-side` / `data-align` 等 signal 保留为 **compat shim**，不作递归保证的基础抽象
- 见 `design.md` §13、`tasks.md` §8

**目标 API（Phase C）**

```tsx
function NestedOverlayMenu() {
  const container = useSpatialPortalContainer()

  return (
    <DropdownMenu.Content asChild sideOffset={8}>
      <SpatialOverlay style={{ '--xr-back': 80, '--xr-depth': 40 }}>
        <DropdownMenu.Item>Archive</DropdownMenu.Item>
        <DropdownMenu.Sub>
          <DropdownMenu.SubTrigger>More</DropdownMenu.SubTrigger>
          {/* D1: inner Portal must target parent visible overlay webview (B.body), not root SpatialDiv (A.body) */}
          <DropdownMenu.Portal container={container}>
            <DropdownMenu.SubContent asChild>
              <SpatialOverlay style={{ '--xr-back': 12, '--xr-depth': 20 }}>
...
**BREAKING**: none（Phase C 为 additive；`div enable-xr` compat 路径保留至 deprecation）
...
6. ~~首版 MVP 对 nesting 深度、modal 模式、动画的约束边界？~~ → Phase B MVP 仅单层 menu；**Phase C 必须支持 `SpatialOverlay` 递归嵌套**（深度无硬编码上限，受 native/webview 资源约束）
