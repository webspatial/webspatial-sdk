...
- **THEN** SDK MUST 在 React prop 绑定层检测到 `animation` prop 出现在非 `Spatialized2DElementContainer` 的 DOM 元素上时发出 warning
- **AND** SDK MUST NOT 启动 native 播放
- **AND** `api.play()` MUST 为空操作（会话 MUST NOT 进入 `queued` 状态；`api.isAnimating` MUST 保持 `false`）
- **AND** `api.pause()` 和 `api.cancel()` MUST 同样为空操作
- **AND** 生命周期回调（`onStart`、`onComplete`、`onCancel`、`onError`）MUST NOT 被触发
...
- **THEN** SDK MUST 先取消 `animationA` 的会话（如有），并调用 `animationA` 的 `onCancel`
- **AND** 若 `animationB` 存在且其 `autoStart` 为 `true`（或未设置），SDK MUST 在取消旧会话后为 `animationB` 启动新会话，并调用 `animationB` 的 `onStart`
- **AND** 旧会话的 `onCancel` MUST 在新会话的 `onStart` 之前触发
- **AND** 旧会话的 cancel 命令 MUST 在新会话的 play 命令之前发送至 native bridge
...
`SpatialDiv` 动画 MUST 仅支持不改变 DOM 布局盒、native 空间面板尺寸、深度或空间位置语义的视觉字段：`transform.translate.x`、`transform.translate.y`、`transform.translate.z`、`transform.rotate.x`、`transform.rotate.y`、`transform.rotate.z`、`transform.scale.x`、`transform.scale.y`、`transform.scale.z`、`opacity`。
...
#### Scenario: transform 仅支持视觉 SRT 分量

- **WHEN** 动画配置包含 `transform`
- **THEN** SDK MUST 仅接受 `translate?: { x?: number, y?: number, z?: number }`、`rotate?: { x?: number, y?: number, z?: number }`、`scale?: { x?: number, y?: number, z?: number }` 这三个结构化子字段
- **AND** `rotate.x/y/z` MUST 分别对齐 CSS `rotateX()`、`rotateY()`、`rotateZ()`，单位为 degree
- **AND** `scale.x/y/z` MUST 分别对齐 CSS `scaleX()`、`scaleY()`、`scaleZ()`，为无单位倍率
- **AND** SDK MUST 以固定顺序组合 transform：translate → rotate → scale
- **AND** SDK MUST NOT 将其解释为任意 CSS transform 字符串、`skew`、`perspective` 或矩阵插值

#### Scenario: 拒绝影响布局或空间尺寸的字段

- **WHEN** 动画配置包含 `width`、`height`、`back`、`backOffset`、`depth` 或其他会影响布局、空间面板尺寸、深度、空间位置语义的字段
- **THEN** SDK MUST 直接抛错
- **AND** SDK MUST NOT 静默忽略这些字段或降级为普通 DOM / CSS 更新
...
- **GIVEN** 应用代码在 `to` 中同时传入了 entity key（`position`、`rotation`、`scale`）和 SpatialDiv key（`transform`、`opacity`）
- **WHEN** SDK 校验配置
- **THEN** SDK MUST 直接抛错
...
#### Scenario: cancel 或自然完成后 isAnimating 为 false
...
播放 API MUST 允许应用启动、暂停、取消 `SpatialDiv` 动画会话，并 MUST 提供 `onStart`、`onComplete`、`onCancel`、`onError` 生命周期。
...
- **GIVEN** `SpatialDiv` 已开启 `enable-xr` 但尚未完成 native 绑定
- **WHEN** 应用调用 `api.play()`
...
- **AND** 若排队期间调用 `api.cancel()`，SDK MUST 取消排队的 play 并触发 `onCancel`
...
#### Scenario: delay 期间暂停与继续

- **GIVEN** 动画配置设置了正数 `delay`，且播放已请求
- **WHEN** 应用先调用 `api.pause()` 再调用 `api.play()`
- **THEN** 剩余 delay 时间 MUST 被保留
- **AND** 继续后 MUST 从暂停点继续，而不是重新开始完整 delay

#### Scenario: delay 期间暂停后 cancel

- **GIVEN** 动画会话处于 delay 阶段
- **AND** 应用调用了 `api.pause()`
- **WHEN** 应用随后调用 `api.cancel()`
- **THEN** `onCancel` MUST 触发一次
- **AND** `onCancel` MUST 收到恢复后的值

#### Scenario: delay 期间直接 cancel
...
- **WHEN** 应用调用 `api.cancel()`
- **THEN** `onCancel` MUST 收到恢复后的值
- **AND** 返回的 `SpatialDivAnimatedValues` MUST 仅包含 `to` 中声明的字段对应的恢复值；`to` 中未声明的字段 MUST NOT 出现在返回值中

#### Scenario: 回调互斥与计数

- **WHEN** `api.play()` 启动一个动画会话
- **THEN** `onStart` MUST 对该会话至多调用一次
- **AND** 会话结束时，`onComplete` 与 `onCancel` MUST 互斥，各自对该会话至多调用一次

#### Scenario: 异步失败回调

- **WHEN** bridge 或 native 的 `play`、`pause`、`resume`、`cancel` 命令异步失败
...
- **AND** 若失败命令是 `play`，`onStart`、`onComplete`、`onCancel` MUST NOT 被调用
- **AND** 若失败命令是 `pause`、`resume`、`cancel`，该失败本身 MUST NOT 触发 `onComplete` 或 `onCancel`
...
- **GIVEN** hook 实例快速连续调用 `play`、`pause`、`cancel` 等控制方法
...
- **GIVEN** 一个动画会话控制 `opacity`
- **WHEN** `PortalInstanceObject` 在会话 alive 期间执行常规属性同步
- **THEN** SDK MUST 抑制 `opacity` 的普通同步
...
#### Scenario: cancel 恢复到 from

- **GIVEN** 一个动画会话处于中间态
- **WHEN** 应用调用 `api.cancel()`
- **THEN** `SpatialDiv` MUST 恢复到 `from`（若省略 `from` 则恢复到起始快照）
- **AND** SDK MUST NOT 让其停留在 cancel 点
...
- **AND** 该 `animationId` 后续 MUST NOT 再收到 `_completed` 或 `_canceled`
...
- **AND** 该失败 MUST NOT 终止 alive 会话；`_completed` 或 `_canceled` 后续仍 MAY 到达
...
- **AND** 卸载后 MUST NOT 再触发 `onStart`、`onComplete`、`onCancel`、`onError`
