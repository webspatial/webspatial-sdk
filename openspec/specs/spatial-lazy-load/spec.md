...
Define the lazy-load architecture for `@webspatial/react-sdk`: a tiny default entry with facades and `bootSpatial()`-gated dynamic import of the spatial implementation, plus packaging hygiene contracts shared with the eager entry.
